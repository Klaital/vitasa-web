...
class SuggestionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create suggestion when not logged in" do
    assert_difference('Suggestion.count', 1) do
      post suggestions_path, params: { suggestion: { 
  ...
  test "should update suggestion when logged in as the owning user" do
    ...
    patch suggestion_path(@suggestion), params: { suggestion: { 
  ...
  test "should delete suggestion when logged in as an admin" do
    u = users(:two)
    u.organization = @suggestion.user.organization
    u.save
    cookie = login_user('user-two', ['Admin'])
    assert_difference('Suggestion.count', -1) do
      delete suggestion_path(@suggestion), headers: {
          'Cookie': cookie,
      }
    end
    assert_response :success
  ...
  test "should delete suggestion when logged in as a superadmin" do
    cookie = login_user('user-two', ['SuperAdmin'])
    assert_difference('Suggestion.count', -1) do
      delete suggestion_path(@suggestion), headers: {
          'Cookie': cookie,
      }
